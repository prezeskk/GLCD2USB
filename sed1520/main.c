...
uchar usbFunctionWrite(uchar *data, uchar len)
{
  ...
  case GLCD2USB_RID_WRITE:
    ...
    if(cmd_state.offset == 0xffff) {
      ...
      /* set draw cursor */
      glcdSetAddress(cmd_state.offset%GLCD_XPIXELS, cmd_state.offset/GLCD_XPIXELS);
    }
...
void whirl_init(void) {
  ...
  rprintf("Driver:SED1520 V%d.%02x", VERSION_MAJOR, VERSION_MINOR);
